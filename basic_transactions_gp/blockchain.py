...
def validate_proof():
    if 'proof' in request.get_json() and 'id' in request.get_json():
        ...
        if blockchain.valid_proof(block_string, proof):
            ...
            response = {
                'message': "sorry, that is not a valid proof"
            }
        status = 200
    else:
        response = {
            'message': 'Error: please make sure your request has an id and a proof'
        }
        status = 400
...
def new_transaction():
    ...
    if not all(k in values for k in required):
        ...
        response =  {'message': f"Transaction with be added to block {index}"}
        status = 200
    return jsonify(response), status
...
"""
Holds a sequence of elements and occupies a contiguous block of memory. 
"""